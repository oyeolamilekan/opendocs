...
- **Base URL** — the API base, e.g. `https://api.acme.dev`. Shown on endpoint pages and used when readers copy example requests.
...
`/app/<org>/projects/<project>/versions` is where you manage versions. Each project has one `isDefault` version used to resolve `/docs/...` and `/reference/...` URLs without a version segment.
...
`/app/<org>/projects/<project>/reference/<endpoint>` is the endpoint editor. The reference is organized into **sections**; each section holds ordered endpoints.
...
Guides are long-form articles that sit alongside the reference. `/app/<org>/projects/<project>/guides/<guide>` is the editor. Like the reference, guides are organized into ordered **sections**, each holding ordered **pages**.
...
4. Visit `http://<project-slug>.<VITE_PUBLIC_DOCS_ROOT_DOMAIN>` (or `http://<project-slug>.localhost:3000` locally) to see the published site.
